var gulp = require( 'gulp' ),
    ...
    onError = function ( error ) {
        gutil.log(
            gutil.colors.red( 'ERROR from ' + error.plugin ) + ':',
            (error.messageOriginal||error.message||'').replace( /\.$/, '' ) + (!!error.cause && !!error.cause.message ? ' (' + error.cause.message + ')' : ''),
            typeof error.line !== 'undefined' ? 'in ' + (error.relativePath) + ' (line ' + error.line + ')' : ''
        );
        this.emit( 'end' );
    };
